В этом статье затрагивается тема построения правильной архитектуры приложения. На примере небольшого проекта, мы разберем основные принципы построения более сложных приложений. Материал будет полезен тем, кто либо никогда не работал над крупными проектами, либо работал над ними, но испытывал сложности в формирвании архитектуры. Статья хорошо подойдет студентам, которые приступают к работе над первым проектом.
...
it('test', () => {
  expect(app()).toBeTrhuthy();
});
```

## Выделяем общую логику

Наше приложение будет спрашивать у пользователя путь к файлу и затем выводить содержимое.
...
Очень важно выделить логические шаги нашего приложения, на их основе будут строиться модулию нашего приложения. К сожалению, даже на этом этапе можно допустить ошибку. Например, новички часто совмещают некоторые шаги и получается:
...
Есть и другие форматы данных, для них используются разные инструменты парсинга. Сейчас это нам не важно. Вам нужно лишь понимать, что наша задача — это получить объект или массив из строки, которая храниться в файле.
...
Но такой подход не очень хороший, так как вызов содержит логику — предачу параметра. Приходится думать какой параметр передать. Вся логика должна находиться внутри функции, а исполняемый код просто вызывает эту фукнцию.
